  ...
  const generateReport = async () => {
    ...
    try {
      ...
      reportElement.innerHTML = `
        <div style="border: 2px solid #000; padding: 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <thead>
                ...
                <tr>
                  ...
                  <th style="border: 1px solid #000; padding: 5px; background-color: #f0f0f0;">OK</th>
                  <th style="border: 1px solid #000; padding: 5px; background-color: #f0f0f0;"></th>
                  <th style="border: 1px solid #000; padding: 5px; background-color: #f0f0f0;">OK</th>
                  <th style="border: 1px solid #000; padding: 5px; background-color: #f0f0f0;"></th>
                  <th style="border: 1px solid #000; padding: 5px; background-color: #f0f0f0;">OK</th>
              ...
              <tbody>
                ${Array.from({ length: Math.max(7, Math.ceil((formData.testeZeroGraos?.length || 0) / 4)) }, (_, rowIndex) => {
                  const startIndex = rowIndex * 4
                  const rowTests = formData.testeZeroGraos?.slice(startIndex, startIndex + 4) || []
                  
                  return `<tr>
                    ${Array.from({ length: 4 }, (_, colIndex) => {
                      const teste = rowTests[colIndex]
                      return `
                        <td style="border: 1px solid #000; padding: 5px; text-align: center;">${teste?.horario || ''}</td>
                        <td style="border: 1px solid #000; padding: 5px; text-align: center;">${teste?.status || ''}</td>
                      `
                    }).join('')}
                  </tr>`
                }).join('')}
          ...
          <div style="text-align: center; font-weight: bold; margin-top: 20px;">
            Tempo Efetivo de Trabalho: ${formData.tempoEfetivo || '00:00'}
          </div>
        </div>
      `
      ...
      if (selectedFormat === 'pdf') {
        ...
        const fileName = `controle-producao-${formData.data || 'sem-data'}.pdf`
        pdf.save(fileName)
        ...
        canvas.toBlob((blob) => {
          ...
          link.href = url
          link.download = `controle-producao-${formData.data || 'sem-data'}.jpg`
